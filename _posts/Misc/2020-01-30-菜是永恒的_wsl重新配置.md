...
今天跟hxl打misc红包题，最后一关pwn给我了，我随手写了个脚本（高看了自己，以为自己ret2libc可以轻松拿下），然后命名为**pwn.py**，运行，然后，boom！pwntools给搞崩了。然后我以为重新安装就行了，于是：
...
```
surager@KaliWindows:~$ sudo apt install cmatrix
```

小火车：

```
surager@KaliWindows:~$ sudo apt install sl
```

好莱坞黑客帝国：

```
surager@KaliWindows:~$ sudo apt install hollywood
```

显示版本：

```
surager@KaliWindows:~$ sudo apt install screenfetch
```

linuxlogo：

```
surager@KaliWindows:~$ sudo apt install linuxlogo
```
...
```
surager@KaliWindows:~$ sudo apt install cowsay
```

小火苗：

```
surager@KaliWindows:~$ sudo apt install libaa-bin
surager@KaliWindows:~$ aafire
```
